...
subroutine h_psi_meta (ldap, np, mp, psip, hpsi)
  ...
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_h_psi_meta', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "kedtau = ", kedtau    !FZ: test
    WRITE(unit, *)  "xk = ", xk    !FZ: test
    WRITE(unit, *)  "g = ", g    !FZ: test
    WRITE(unit, *)  "mp = ", mp    !FZ: test
    WRITE(unit, *)  "dffts%nl = ", dffts%nl    !FZ: test
    WRITE(unit, *)  "psip = ", psip    !FZ: test
    WRITE(unit, *)  "hpsi = ", hpsi    !FZ: test
    WRITE(unit, *)  "np = ", np    !FZ: test
    WRITE(unit, *)  "current_k = ", current_k    !FZ: test
  ...
  if (gamma_only) then
     ...
     do im = 1, mp, 2
        do j =1,3
           ...
           !
           psic(1:nrxxs) = kedtau(1:nrxxs,current_spin) * psic(1:nrxxs)   !FZ: correction  from metaGGA to h * psi
           !
