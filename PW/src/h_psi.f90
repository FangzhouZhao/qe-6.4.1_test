
! Copyright (C) 2002-2016 Quantum ESPRESSO group
! This file is distributed under the terms of the
! GNU General Public License. See the file `License'
! in the root directory of the present distribution,
! or http://www.gnu.org/copyleft/gpl.txt .
!
!----------------------------------------------------------------------------
SUBROUTINE h_psi( lda, n, m, psi, hpsi )
  !----------------------------------------------------------------------------
  !
  ! ... This routine computes the product of the Hamiltonian
  ! ... matrix with m wavefunctions contained in psi
  !
  ! ... input:
  ! ...    lda   leading dimension of arrays psi, spsi, hpsi
  ! ...    n     true dimension of psi, spsi, hpsi
  ! ...    m     number of states psi
  ! ...    psi
  !
  ! ... output:
  ! ...    hpsi  H*psi
  !
  ! --- Wrapper routine: performs bgrp parallelization on non-distributed bands
  ! --- if suitable and required, calls old H\psi routine h_psi_
  !
  USE kinds,            ONLY : DP
  USE noncollin_module, ONLY : npol    !FZ: !  number of coordinates of wfc
  USE funct,            ONLY : exx_is_active
  USE mp_bands,         ONLY : use_bgrp_in_hpsi, inter_bgrp_comm
  USE mp,               ONLY : mp_allgather, mp_size, &
                               mp_type_create_column_section, mp_type_free
  USE io_global, ONLY : stdout, ionode     !FZ: test
  USE wvfct,    ONLY : g2kin             !FZ: test
  !
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN)      :: lda, n, m
  COMPLEX(DP), INTENT(IN)  :: psi(lda*npol,m) 
  COMPLEX(DP), INTENT(OUT) :: hpsi(lda*npol,m)
  !
  INTEGER :: m_start, m_end
  INTEGER :: column_type
  INTEGER, ALLOCATABLE :: recv_counts(:), displs(:)
  !
  integer :: unit   !FZ: test
  unit = 4   !FZ: test
  CALL start_clock( 'h_psi_bgrp' ); !write (*,*) 'start h_psi_bgrp'; FLUSH(6)

  ! band parallelization with non-distributed bands is performed if
  ! 1. enabled (variable use_bgrp_in_hpsi must be set to .T.)
  ! 2. exact exchange is not active (if it is, band parallelization is already
  !    used in exx routines called by Hpsi)
  ! 3. there is more than one band, otherwise there is nothing to parallelize
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_start_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi1 = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF   !FZ: test
  ENDIF   !FZ: test
  IF (use_bgrp_in_hpsi .AND. .NOT. exx_is_active() .AND. m > 1) THEN
     ! use band parallelization here
     ALLOCATE( recv_counts(mp_size(inter_bgrp_comm)), displs(mp_size(inter_bgrp_comm)) )
     CALL divide_all(inter_bgrp_comm,m,m_start,m_end,recv_counts,displs)
     CALL mp_type_create_column_section(hpsi(1,1), 0, lda*npol, lda*npol, column_type)
     !
     ! Check if there at least one band in this band group
     IF (m_end >= m_start) &
        CALL h_psi_( lda, n, m_end-m_start+1, psi(1,m_start), hpsi(1,m_start) )
     CALL mp_allgather(hpsi, column_type, recv_counts, displs, inter_bgrp_comm)
     !
     CALL mp_type_free( column_type )
     DEALLOCATE( recv_counts )
     DEALLOCATE( displs )
  ELSE
     ! don't use band parallelization here
     CALL h_psi_( lda, n, m, psi, hpsi )
  END IF

  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_final_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi1 = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF
  ENDIF
  CALL stop_clock( 'h_psi_bgrp' )
  RETURN
  !
END SUBROUTINE h_psi
!
!----------------------------------------------------------------------------
SUBROUTINE h_psi_( lda, n, m, psi, hpsi )
  !----------------------------------------------------------------------------
  !
  ! ... This routine computes the product of the Hamiltonian
  ! ... matrix with m wavefunctions contained in psi
  !
  ! ... input:
  ! ...    lda   leading dimension of arrays psi, spsi, hpsi
  ! ...    n     true dimension of psi, spsi, hpsi
  ! ...    m     number of states psi
  ! ...    psi
  !
  ! ... output:
  ! ...    hpsi  H*psi
  !
  USE kinds,    ONLY : DP
  USE bp,       ONLY : lelfield,l3dstring,gdir, efield, efield_cry
  USE becmod,   ONLY : bec_type, becp, calbec
  USE lsda_mod, ONLY : current_spin
  USE scf,      ONLY : vrs  
  USE wvfct,    ONLY : g2kin
  USE uspp,     ONLY : vkb, nkb
  USE ldaU,     ONLY : lda_plus_u, U_projection
  USE gvect,    ONLY : gstart
  USE funct,    ONLY : dft_is_meta
  USE control_flags,    ONLY : gamma_only
  USE noncollin_module, ONLY: npol, noncolin
  USE realus,   ONLY : real_space, &
                       invfft_orbital_gamma, fwfft_orbital_gamma, calbec_rs_gamma, add_vuspsir_gamma, & 
                       invfft_orbital_k, fwfft_orbital_k, calbec_rs_k, add_vuspsir_k, & 
                       v_loc_psir_inplace
  USE fft_base, ONLY : dffts
  USE exx,      ONLY : use_ace, vexx, vexxace_gamma, vexxace_k
  USE funct,    ONLY : exx_is_active
  USE fft_helper_subroutines
  USE io_global, ONLY : stdout, ionode     !FZ: test
  !
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN)     :: lda, n, m
  COMPLEX(DP), INTENT(IN)  :: psi(lda*npol,m) 
  COMPLEX(DP), INTENT(OUT) :: hpsi(lda*npol,m)   
  !
  INTEGER     :: ipol, ibnd
  REAL(dp)    :: ee
  integer :: unit   !FZ: test
  unit = 4   !FZ: test
  !
  CALL start_clock( 'h_psi' ); !write (*,*) 'start h_psi';FLUSH(6)
  !
  ! ... Here we add the kinetic energy (k+G)^2 psi and clean up garbage
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ:  test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_0', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
      WRITE (unit, *) "psi = ", psi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF    !FZ:  test
  ENDIF    !FZ:  test
  !hpsi(:,:) = (0.0D0, 0.0D0)  !FZ: test
  !FZ: test result Important! This line justifies the hpsi_0 starting values are useless, the true begining to initialize hpsi starts below!
  !$omp parallel do
  DO ibnd = 1, m
     hpsi (1:n, ibnd) = g2kin (1:n) * psi (1:n, ibnd)
     IF (n<lda) hpsi (n+1:lda, ibnd) = (0.0_dp, 0.0_dp)
     IF ( noncolin ) THEN
        hpsi (lda+1:lda+n, ibnd) = g2kin (1:n) * psi (lda+1:lda+n, ibnd)
        IF (n<lda) hpsi (lda+n+1:lda+lda, ibnd) = (0.0_dp, 0.0_dp)
     END IF
  END DO
  !$omp end parallel do
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF
  ENDIF
  !
  ! ... Here the product with the local potential V_loc psi
  !
  CALL start_clock( 'h_psi:pot' ); !write (*,*) 'start h_pot';FLUSH(6)
  !
  IF ( gamma_only ) THEN
     ! 
     IF ( real_space .and. nkb > 0  ) then 
        !
        ! ... real-space algorithm
        ! ... fixme: real_space without beta functions does not make sense
        !
        IF ( dffts%has_task_groups ) &
             CALL errore( 'h_psi', 'task_groups not implemented with real_space', 1 )

        DO ibnd = 1, m, 2
           ! ... transform psi to real space -> psic 
           CALL invfft_orbital_gamma(psi,ibnd,m) 
           ! ... compute becp%r = < beta|psi> from psic in real space
     CALL start_clock( 'h_psi:calbec' ) 
           CALL calbec_rs_gamma(ibnd,m,becp%r) 
     CALL stop_clock( 'h_psi:calbec' )
           ! ... psic -> vrs * psic (psic overwritten will become hpsi)
           CALL v_loc_psir_inplace(ibnd,m) 
           ! ... psic (hpsi) -> psic + vusp
           CALL  add_vuspsir_gamma(ibnd,m)
           ! ... transform psic back in reciprocal space and assign it to hpsi
           CALL fwfft_orbital_gamma(hpsi,ibnd,m) 
        END DO
        !
     ELSE
        ! ... usual reciprocal-space algorithm
        CALL vloc_psi_gamma ( lda, n, m, psi, vrs(1,current_spin), hpsi )    !FZ: hpsi = vrs * psi 
        !
     ENDIF 
     !IF (ionode) THEN                                          !FZ: test
     !  OPEN (unit = unit, file = 'test_h_psi_gammaonly', &          !FZ: test
     !    form = 'formatted', status = 'replace')               !FZ: test
     !    WRITE (unit, *) "hpsi_gammaonly = ", hpsi  !FZ:  test
     !  CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
     !ENDIF
     !
  ELSE IF ( noncolin ) THEN 
     !
     CALL vloc_psi_nc ( lda, n, m, psi, vrs, hpsi )
     !
  ELSE  
     ! 
     IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
     IF (ionode) THEN                                          !FZ: test
       OPEN (unit = unit, file = 'test_h_psi_g2kin2', &          !FZ: test
         form = 'formatted', status = 'replace')               !FZ: test
         WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
         WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
         WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
       CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
     ENDIF  !FZ: test
     ENDIF  !FZ: test
     IF ( real_space .and. nkb > 0  ) then 
        !
        ! ... real-space algorithm
        ! ... fixme: real_space without beta functions does not make sense
        !
        IF ( dffts%has_task_groups ) &
             CALL errore( 'h_psi', 'task_groups not implemented with real_space', 1 )

        DO ibnd = 1, m
           ! ... transform psi to real space -> psic 
           CALL invfft_orbital_k(psi,ibnd,m) 
           ! ... compute becp%r = < beta|psi> from psic in real space
     CALL start_clock( 'h_psi:calbec' )
           CALL calbec_rs_k(ibnd,m) 
     CALL stop_clock( 'h_psi:calbec' )
           ! ... psic -> vrs * psic (psic overwritten will become hpsi)
           CALL v_loc_psir_inplace(ibnd,m) 
           ! ... psic (hpsi) -> psic + vusp
           CALL  add_vuspsir_k(ibnd,m)
           ! ... transform psic back in reciprocal space and assign it to hpsi
           CALL fwfft_orbital_k(hpsi,ibnd,m) 
        END DO
        !
        IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
        IF (ionode) THEN                                          !FZ: test
          OPEN (unit = unit, file = 'test_h_psi_g2kin3', &          !FZ: test
            form = 'formatted', status = 'replace')               !FZ: test
            WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
            WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
            WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
          CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
        ENDIF !FZ: test
        ENDIF !FZ: test
        !FZ: test result: this real space algorithm is not called
     ELSE
        !
        CALL vloc_psi_k ( lda, n, m, psi, vrs(1,current_spin), hpsi )    !FZ: vrs is the potential (psi + kin + hartree + ..) in real space
        !
        IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
        IF (ionode) THEN                                          !FZ: test
          OPEN (unit = unit, file = 'test_h_psi_g2kin4', &          !FZ: test
            form = 'formatted', status = 'replace')               !FZ: test
            WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
            WRITE (unit, *) "vrs = ", vrs  !FZ:  test
            WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
            WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
          CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
        ENDIF !FZ: test
        ENDIF !FZ: test
     END IF  
     !IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN  !FZ:  test
     !IF (ionode) THEN                                          !FZ: test
     !  OPEN (unit = unit, file = 'test_h_psi_k', &          !FZ: test
     !    form = 'formatted', status = 'replace')               !FZ: test
     !    WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
     !    WRITE (unit, *) "hpsi_k = ", hpsi  !FZ:  test
     !  CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
     !ENDIF  !FZ:  test
     !ENDIF  !FZ:  test
     !
  END IF  
  !
  ! ... Here the product with the non local potential V_NL psi
  ! ... (not in the real-space case: it is done together with V_loc)
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ:  test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_aftervrs', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin+vrs = ", hpsi  !FZ:  test
      WRITE (unit, *) "psi = ", psi  !FZ:  test
      !WRITE (unit, *) "becp = ", becp  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF     !FZ:  test
  ENDIF     !FZ:  test
  IF ( nkb > 0 .AND. .NOT. real_space) THEN
     !
     CALL start_clock( 'h_psi:calbec' )
     CALL calbec ( n, vkb, psi, becp, m )   !FZ: becp  ! <beta|psi>
     CALL stop_clock( 'h_psi:calbec' )
     CALL add_vuspsi( lda, n, m, hpsi )
     !
  END IF
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin5', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  !  
  CALL stop_clock( 'h_psi:pot' )
  !
  if (dft_is_meta()) call h_psi_meta (lda, n, m, psi, hpsi)   !FZ: add specific contribution from metaGGA to hpsi (when call this, it is added to hpsi)
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin6', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  !
  ! ... Here we add the Hubbard potential times psi
  !
  IF ( lda_plus_u .AND. U_projection.NE."pseudo" ) THEN
     !
     IF (noncolin) THEN
        CALL vhpsi_nc( lda, n, m, psi, hpsi )
     ELSE
        call vhpsi( lda, n, m, psi, hpsi )
     ENDIF
     !
  ENDIF
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin7', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  ! ... Here the exact-exchange term Vxx psi
  !
  IF ( exx_is_active() ) THEN
     IF ( use_ace) THEN
        IF (gamma_only) THEN
           CALL vexxace_gamma(lda,m,psi,ee,hpsi)
        ELSE
           CALL vexxace_k(lda,m,psi,ee,hpsi) 
        END IF
     ELSE
        CALL vexx( lda, n, m, psi, hpsi, becp )
     END IF
  END IF
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin8', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  ! ... electric enthalpy if required
  !
  IF ( lelfield ) THEN
     !
     IF ( .NOT.l3dstring ) THEN
        CALL h_epsi_her_apply( lda, n, m, psi, hpsi,gdir, efield )
     ELSE
        DO ipol=1,3
           CALL h_epsi_her_apply( lda, n, m, psi, hpsi,ipol,efield_cry(ipol) )
        END DO
     END IF
     !
  END IF
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin9', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  ! ... With Gamma-only trick, Im(H*psi)(G=0) = 0 by definition,
  ! ... but it is convenient to explicitly set it to 0 to prevent trouble
  !
  IF ( gamma_only .AND. gstart == 2 ) &
      hpsi(1,1:m) = CMPLX( DBLE( hpsi(1,1:m) ), 0.D0 ,kind=DP)
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN   !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_h_psi_g2kin10', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
      WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi_g2kin = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF !FZ: test
  ENDIF !FZ: test
  CALL stop_clock( 'h_psi' )
  !
  RETURN
  !
END SUBROUTINE h_psi_
