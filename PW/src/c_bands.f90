...
integer :: unit   !FZ: test
  ...
  !
  npw = ngk(ik)    !FZ: npw is number of G vectors at k point
  IF ( gamma_only ) THEN
     !
  ...
  ELSE
     !
     CALL diag_bands_k()    !FZ: commented
     !CALL diag_bands_k(ik)   !FZ: test: give ik value to the cegterg and hpsi function inside, to only output the gamma point information
     !
  END IF
  !
...
CONTAINS
  ...
  SUBROUTINE diag_bands_gamma()
    ...
    ELSE
       ...
       !
       CALL usnldiag( npw, h_diag, s_diag )  !FZ: add nonlocal pseudopotential term to diagonal part of Hamiltonian    compute the diagonal part of the S matrix
       !
       ntry = 0
       !
       david_loop: DO
...
!             ! make sure that all processors have the same wfc
  ...
  SUBROUTINE diag_bands_k()   
    ...
    IF (ionode) THEN                                          !FZ: test
      OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_1', &          !FZ: test
        form = 'formatted', status = 'replace')               !FZ: test
        WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
        !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
    ...
    IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
    IF (ionode) THEN                                          !FZ: test
      OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_2', &          !FZ: test
        form = 'formatted', status = 'replace')               !FZ: test
        WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
        !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
        WRITE (unit, *) "h_diag = ", h_diag  !FZ:  test
...
! BEWARE npol should be added to the arguments
    ...
    ELSE
       ...
       IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
       IF (ionode) THEN                                          !FZ: test
         OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_3', &          !FZ: test
           form = 'formatted', status = 'replace')               !FZ: test
           WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
           !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
           WRITE (unit, *) "h_diag = ", h_diag  !FZ:  test
       ...
       IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
       IF (ionode) THEN                                          !FZ: test
         OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_4', &          !FZ: test
           form = 'formatted', status = 'replace')               !FZ: test
           WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
           !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
           WRITE (unit, *) "h_diag = ", h_diag  !FZ:  test
       ...
       CALL usnldiag( npw, h_diag, s_diag )   !FZ: add nonlocal pseudopotential term to diagonal part of Hamiltonian
                                              !FZ: compute the diagonal part of the S matrix
       !
       IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
       IF (ionode) THEN                                          !FZ: test
         OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_5', &          !FZ: test
           form = 'formatted', status = 'replace')               !FZ: test
           WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
           !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
           WRITE (unit, *) "h_diag = ", h_diag  !FZ:  test
       ...
       david_loop: DO
          ...
          IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN     !FZ: test
          IF (ionode) THEN                                          !FZ: test
            OPEN (unit = unit, file = 'test_diag_bandsk_hdiag_output_6', &          !FZ: test
              form = 'formatted', status = 'replace')               !FZ: test
              WRITE (unit, *) "g2kin(1) = ", g2kin(1)  !FZ:  test
              !WRITE (unit, *) "lda = ", lda, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
              WRITE (unit, *) "h_diag = ", h_diag  !FZ:  test
  ...
  FUNCTION test_exit_cond()
    ...
    !
    !
    test_exit_cond = .NOT. ( ( ntry <= 5 ) .AND. &    !FZ: either ntry > 5, or both (lscf .AND. ( notconv > 0 )) = true and ( lscf .AND. ( notconv > 5 )) = false
...
SUBROUTINE c_bands_nscf( )
  ...
  USE mp,                   ONLY : mp_sum
  USE check_stop,           ONLY : check_stop_now
  USE io_global,        ONLY : ionode, ionode_id, stdout    !FZ: test
  USE scf,      ONLY : vrs  !FZ: test
  !
  IMPLICIT NONE
  !
  ...
  avg_iter = 0.D0
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_cbands_nscf_vrs_1', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
  ...
  !
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_cbands_nscf_vrs_2', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "vrs = ", vrs    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  END DO
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_cbands_nscf_vrs_3', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "vrs = ", vrs    !FZ: test
  ...
  k_loop: DO ik = ik_+1, nks
     ...
     ! ... Set k-point, spin, kinetic energy, needed by Hpsi
     !
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_4', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_5', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       WRITE(unit, *)  "nkb = ", nkb    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ENDIF                                       !FZ:  test
     ! ... More stuff needed by the hamiltonian: nonlocal projectors
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_6', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_7', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ENDIF                                       !FZ:  test
     !FZ: test result: here et is 0 , does not have values
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_8', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       WRITE(unit, *)  "evc = ", evc    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ENDIF                                       !FZ:  test
     !FZ: test result: here et has wrong eigenvalues
     !
     ! ... diagonalization of bands for k-point ik
     !
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_9', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       WRITE(unit, *)  "evc = ", evc    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ...
     ENDIF                                       !FZ:  test
     !
     !FZ: test result: here et has right eigenvalues
     !FZ: test result: here evc has the final eigenvectors this evc is same as what is read in pw2bgw, this is final
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_10', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
     ...
     IF (ik == ik_+1) THEN    !FZ: test
     IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_vrs_11', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "vrs = ", vrs    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
  ...
  WRITE( stdout, '(/,5X,"ethr = ",1PE9.2,",  avg # of iterations =",0PF5.1)' ) &
       ethr, avg_iter
  IF (ionode) THEN                                          !FZ: test
        OPEN (unit = unit, file = 'test_cbands_nscf_et_12', &  !FZ: test
          form = 'formatted', status = 'replace')               !FZ: test
       WRITE(unit, *)  "et = ", et    !FZ: test
       CLOSE (unit = unit, status = 'keep')      !FZ:  test
     ENDIF                                       !FZ:  test
  !
