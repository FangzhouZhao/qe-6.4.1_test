...
SUBROUTINE v_of_rho( rho, rho_core, rhog_core, &
  ...
  !
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_2', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_3', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  !
  CALL v_h( rho%of_g(:,1), ehart, charge, v%of_r )    !FZ: add hartree potential to v%of_r
  !
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_4', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  !
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_5', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  !
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_6', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  END IF
  IF (ionode) THEN                                          !FZ: test
     OPEN (unit = unit, file = 'test_v_of_rho_v%of_r_7', &  !FZ: test
       form = 'formatted', status = 'replace')               !FZ: test
    WRITE(unit, *)  "v%of_r = ", v%of_r    !FZ: test
    CLOSE (unit = unit, status = 'keep')      !FZ:  test
  ENDIF                                       !FZ:  test
  ...
  do k = 1, dfftp%nnr
     ...
     if (nspin == 1) then
        ...
        if ( (arho > eps8) .and. (grho2 (1) > eps12) .and. &
           ...
           IF (ionode) THEN                                          !FZ: test
             OPEN (unit = unit, file = 'test_v_xc_meta_output2', &  !FZ: test
               form = 'formatted', status = 'replace')               !FZ: test
...
!$omp             reduction(+:etxc,vtxc), reduction(-:rhoneg)
     DO ir = 1, dfftp%nnr
        ...
        arhox = ABS( rhox )
        !
        !IF ( ionode ) WRITE (stdout, '(" Test that v_xc is called and canbe written in scf.out ")')    !FZ: test
        !IF (ionode) THEN                                          !FZ: test
        !  OPEN (unit = unit, file = 'test_v_of_rho_output_2', &  !FZ: test
        !    form = 'formatted', status = 'replace')               !FZ: test
        IF ( arhox > vanishing_charge ) THEN
           ...
           !
           v(ir,1) = e2*( vx(1) + vc(1) )
           !WRITE (unit, *) "v (ir=1, 1) ", v(1,1) !FZ:  test
           !WRITE (unit, *) "vx(1) = ", vx(1), "vc(1) = ", vc(1) !FZ:  test
           !WRITE (unit, *) "arhox = ", arhox, "ex = ", ex, "ec = ", ec !FZ:  test
           !
...
!FZ:  If we want to look at the Hartree potential solely, we use this v_h_only function
!FZ:  Note for future debug
!FZ:  1. this routine has not consider esm_hartree situation ESM (Effective Screening Medium Method)
  !----------------------------------------------------------------------------
  !   !FZ: calculate total hartree energy (scalar) return ehart as total hartree energy
  ! ... Hartree potential VH(r) from n(G)
      !FZ: aux is some field in G space related to charge density, use aux to calculate a potential v 
  !
  ...
  REAL(DP),  INTENT(OUT) :: v(dfftp%nnr,nspin)    !FZ: changed (INOUT) to (OUT)
  ...
  IF ( do_comp_esm .and. ( esm_bc .ne. 'pbc' ) ) THEN
     ...
     !
     CALL errore ('v_h_only', '!FZ: should not use esm_hartree', 1)   !FZ: for metaGGA
     !
  ELSE
     !
     ehart     = 0.D0
...
SUBROUTINE v_h( rhog, ehart, charge, v )
  ...
  !
  ! ... calculate hartree potential in G-space (NB: V(G=0)=0 )
  !FZ:  Hartree potential is calculated without G=0 term !!!!, We may need to add v_of_0 
  !
  IF ( do_comp_esm .and. ( esm_bc .ne. 'pbc' ) ) THEN
     !
     ! ... calculate modified Hartree potential for ESM
