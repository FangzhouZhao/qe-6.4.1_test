...
!$omp end parallel
 ...
 100  CALL errore('pw_readschemafile',TRIM(errmsg),ierr)
    ...
    SUBROUTINE init_vars_from_schema( what, ierr, output_obj, par_info, gen_info, input_obj )
      ...
      CASE( 'wave' )
         ...
         lwfc  = .TRUE.
         !
      CASE( 'nowave' )    !FZ: typical read_file is this case
         !
         lcell   = .TRUE.
         lpw     = .TRUE.
         lions   = .TRUE.
 ...
 !
    ...
    SUBROUTINE readschema_band_structure( band_struct_obj )
      ...
      DO ik =1, band_struct_obj%ndim_ks_energies
         IF ( band_struct_obj%lsda) THEN
            ...
            et(1:nbnd_up_,ik) = band_struct_obj%ks_energies(ik)%eigenvalues%vector(1:nbnd_up_)*e2     !FZ: read ks eigenvalues to et variable from band_struct_obj%ks_energies(ik)%eigenvalues
         ...
         ELSE 
            ...
            et (1:nbnd_,ik) = band_struct_obj%ks_energies(ik)%eigenvalues%vector(1:nbnd_)*e2       !FZ: read ks eigenvalues to et variable from band_struct_obj%ks_energies(ik)%eigenvalues     e2 is square of electron charge which is 2.0
    ...
    ! 
    !------------------------------------------------------------------------
    SUBROUTINE read_collected_to_evc( dirname )    !FZ: read wfn from file
      !------------------------------------------------------------------------
      !
