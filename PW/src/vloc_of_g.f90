...
! ==============================================
  ...
  do igl = igl0, ngl
     ...
     vloc (igl) = vlcp
  enddo
  vloc (:) = vloc(:) * fpi / omega    !FZ: directly calculate the fourier component of ionic potential from pseudopotentials, vloc stores the G space data
!FZ:   \int n(r')/|r-r'| dr' exp(iGr) r^2 sin (theta) dr ~ \int n(r') * r  dr' exp(iGr) dr, so we see r(ir) * ... above 
  deallocate (aux, aux1)
