...
!
     ...
     new = nr   !FZ:test
     IF( PRESENT( np ) ) THEN   !FZ:test
       IF (np <= 0 .OR. np > nr) &   !FZ:test
           CALL fftx_error__( ' good_fft_order ', ' invalid np ', 1 )  !FZ:test
       DO WHILE( ( ( .NOT. allowed( new ) ) .OR. ( MOD( new, np ) /= 0 ) ) .AND. ( new <= nfftx ) ) !FZ:test
         new = new + 1   !FZ:test
       END DO   !FZ:test
     ELSE   !FZ:test
       DO WHILE( ( .NOT. allowed( new ) ) .AND. ( new <= nfftx ) )   !FZ:test
         new = new + 1   !FZ:test
       END DO   !FZ:test
     END IF   !FZ:test
