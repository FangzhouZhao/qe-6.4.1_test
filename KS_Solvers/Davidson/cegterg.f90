...
SUBROUTINE cegterg( h_psi, s_psi, uspp, g_psi, &
  ...
  conv   = .FALSE.
  !
  CALL threaded_memcpy(psi, evc, nvec*npol*npwx*2)   !FZ: copy the values in evc to psi. ! This is the initialization of psi
  !
  ! ... hpsi contains h times the basis vectors
  !
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN       !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_cegterg_h_psi_0_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)   !FZ: test
      WRITE (unit, *) "lda = ", npwx, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF   !FZ: test
  ENDIF   !FZ: test
  IF (abs( abs(g2kin(1)) - 4.499297163419221E-002 ) .lt. 0.0001_dp) THEN       !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_cegterg_h_psi_0_output_2ndkpt', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)   !FZ: test
      WRITE (unit, *) "lda = ", npwx, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
  ...
  ENDIF   !FZ: test
  ENDIF   !FZ: test
   !FZ: testresult: here hpsi is 16 * 22(npwx) elements, its 8*22 - 11*22 (66 elements) is same as the start data of hpsi when calling h_psi function
  CALL h_psi( npwx, npw, nvec, psi, hpsi )
  !IF ( ionode ) WRITE (6, *)  "ik = ", ik  !FZ: test
  IF ( ionode ) WRITE (6, '(" Test that cegterg is called")')  !FZ: test
  IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN       !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_cegterg_h_psi_firsttime_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "g2kin(1) = ", g2kin(1)   !FZ: test
      WRITE (unit, *) "lda = ", npwx, " npol = ", npol, " m = ", m, " n = ", n  !FZ:  test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
...
! NB: must not call mp_sum over inter_bgrp_comm here because it is done later to the full correction
     ...
     IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN       !FZ: test
     IF (ionode) THEN                                          !FZ: test
       OPEN (unit = unit, file = 'test_cegterg_h_psi_beforesecond_output', &          !FZ: test
         form = 'formatted', status = 'replace')               !FZ: test
         WRITE (unit, *) "g2kin(1) = ", g2kin(1)   !FZ: test
         WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
       CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
     ...
     IF (abs(g2kin(1)) .lt. 0.0001_dp) THEN       !FZ: test
     IF (ionode) THEN                                          !FZ: test
       OPEN (unit = unit, file = 'test_cegterg_h_psi_secondtime_output', &          !FZ: test
         form = 'formatted', status = 'replace')               !FZ: test
         WRITE (unit, *) "g2kin(1) = ", g2kin(1)   !FZ: test
         WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
       CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
...
SUBROUTINE pcegterg(h_psi, s_psi, uspp, g_psi, &  
  ...
  !
  CALL h_psi( npwx, npw, nvec, psi, hpsi )
  IF ( ionode ) WRITE (6, '(" Test that pcegterg is called")')  !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_pcegterg_h_psi_firsttime_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF
  ...
  iterate: DO kter = 1, maxter
     ...
     CALL h_psi( npwx, npw, notcnv, psi(1,1,nb1), hpsi(1,1,nb1) )
     IF (ionode) THEN                                          !FZ: test
       OPEN (unit = unit, file = 'test_pcegterg_h_psi_secondtime_output', &          !FZ: test
         form = 'formatted', status = 'replace')               !FZ: test
