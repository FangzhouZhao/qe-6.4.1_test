...
SUBROUTINE regterg(  h_psi, s_psi, uspp, g_psi, &
  ...
  !
  CALL h_psi( npwx, npw, nvec, psi, hpsi )  !FZ: add g2kin + vrs (= vltot+vh+vxc (+ maybe diagonal term for nonlocal pseudopotential)
  IF ( ionode ) WRITE (6, '(" Test that regterg is called")')  !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_regterg_h_psi_firsttime_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
  ...
  hr(:,:) = 0.D0               !FZ: hr is the reduced Hamiltonian in G space
  sr(:,:) = 0.D0
  vr(:,:) = 0.D0
  !
  ...
  if (n_start .le. n_end) &
  CALL DGEMM( 'T','N', nbase, my_n, npw2, 2.D0 , psi, npwx2, hpsi(1,n_start), npwx2, 0.D0, hr(1,n_start), nvecx )  !FZ: calculate hr = 2*psi*hpsi + 0*hr
  
  !IF (gstart == 2) THEN
  !IF (ionode) THEN                                          !FZ: test
  !  OPEN (unit = unit, file = 'test_ifgstart=2', &          !FZ: test
  !    form = 'formatted', status = 'replace')               !FZ: test
  !    WRITE (unit, *) "hpsi = ",hpsi  !FZ:  test
  !    WRITE (unit, *) "hr = ",hr  !FZ:  test
  !    WRITE (unit, *) "vr = ",vr  !FZ:  test
  !    WRITE (unit, *) "psi = ",psi  !FZ:  test
  !  CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  !ENDIF
  !ENDIF  !FZ: for silicon PZ potential gstart = 2
  IF ( gstart == 2 ) CALL DGER( nbase, my_n, -1.D0, psi, npwx2, hpsi(1,n_start), npwx2, hr(1,n_start), nvecx )   !FZ: calculate hr = - psi * hpsi (where psi and hpsi is 1d vector)
  ...
  ELSE
     !
     if (n_start .le. n_end) &
     CALL DGEMM( 'T','N', nbase, my_n, npw2, 2.D0, psi, npwx2, psi(1,n_start), npwx2, 0.D0, sr(1,n_start), nvecx )  !FZ: sr = psi^T * psi
  ...
  !
  IF ( lrot ) THEN    !FZ: lrot = ( iter == 1 ) , only true for 1st iteration
     !
     DO n = 1, nbase
        !
        e(n) = hr(n,n)
  ...
  ELSE
     ...
     IF( my_bgrp_id == root_bgrp_id ) THEN
        CALL rdiaghg( nbase, nvec, hr, sr, nvecx, ew, vr ) !FZ: calculate eigenvalues(ew) and eigenvectors(vr) of hr * vr = ew * sr * vr
     END IF
     IF( nbgrp > 1 ) THEN
  ...
  END IF
  !
  !FZ: This eigenvectors vr and eigenvalues e is just an approximation in the initialization step of David diagonalization 
  !FZ: denote rho_i an x_i as this fist step approximation of eigenvalue and eigenvector
  !FZ: the calculate the residue r_i = (H - \rho_i * I ) x_i   (I is identity) 
  ! ... iterate
  !
  iterate: DO kter = 1, maxter
     ...
     IF ( uspp ) THEN
        ...
        !FZ: new_psi = 1* spsi * vr   (vr is previous eigenvector matrix, spsi is s* psi  (for non USPP it is psi))
        !
     ELSE
        !
        if (n_start .le. n_end) &
...
! NB: must not call mp_sum over inter_bgrp_comm here because it is done later to the full correction
     ...
     DO np = 1, notcnv
        ...
        !
     END DO
     !FZ: here  - \rho_i * I  x_i is calculated, stored in psi(:, nbase + i)
     !
     if (n_start .le. n_end) &
     CALL DGEMM( 'N','N', npw2, notcnv, my_n, 1.D0, hpsi(1,n_start), npwx2, vr(n_start,1), nvecx, 1.D0, psi(1,nb1), npwx2 )
     !FZ: new_psi = hpsi * vr + new_psi
     !FZ: here r_i = (H - \rho_i * I ) x_i is calculated, so here new_psi (psi) is the residue r_i
     ...
     DO n = 1, notcnv
        !
        psi(:,nbase+n) = psi(:,nbase+n) / SQRT( ew(n) )   !FZ: normalize new_psi
        ! ... set Im[ psi(G=0) ] -  needed for numerical stability
     ...
     !FZ:   calculate hr = 2*psi*hpsi + 0*hr,  but this time stored in hr (n_start, nb1) below the first block
     IF ( gstart == 2 ) CALL DGER( my_n, notcnv, -1.D0, psi(1,n_start), npwx2, hpsi(1,nb1), npwx2, hr(n_start,nb1), nvecx )
     !FZ: calculate hr = - psi * hpsi (where psi and hpsi is 1d vector)
     ...
     ELSE
        ...
        !
     END IF
     !FZ:  calculate sr which is psi * psi , is identity if not use USPP
...
SUBROUTINE pregterg(h_psi, s_psi, uspp, g_psi, &
  ...
  IF ( ionode ) WRITE (6, '(" Test that pregterg is called")')  !FZ: test
  IF (ionode) THEN                                          !FZ: test
    OPEN (unit = unit, file = 'test_pregterg_h_psi_firsttime_output', &          !FZ: test
      form = 'formatted', status = 'replace')               !FZ: test
      WRITE (unit, *) "hpsi0 = ", hpsi  !FZ:  test
    CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
  ENDIF
  ...
  iterate: DO kter = 1, maxter
     ...
     CALL h_psi( npwx, npw, notcnv, psi(1,nb1), hpsi(1,nb1) )
     IF (ionode) THEN                                          !FZ: test
       OPEN (unit = unit, file = 'test_pregterg_h_psi_secondtime_output', &          !FZ: test
         form = 'formatted', status = 'replace')               !FZ: test
         WRITE (unit, *) "hpsi1 = ", hpsi  !FZ:  test
       CLOSE (unit = unit) !, status = 'keep')      !FZ:  test
     ENDIF
     !
     IF ( uspp ) CALL s_psi( npwx, npw, notcnv, psi(1,nb1), spsi(1,nb1) )
