...
CONTAINS
  ...
  SUBROUTINE calbec_k ( npw, beta, psi, betapsi, nbnd )
    ...
    IF (ionode) THEN                                          !FZ: test
      OPEN (unit = unit, file = 'test_bec_output', &          !FZ: test
        form = 'formatted', status = 'replace')               !FZ: test
        WRITE (unit, *) "npwx = ", npwx  !FZ:  test
        WRITE (unit, *) "size(psi,1)= ", size (psi, 1)  !FZ:  test
        WRITE (unit, *) "nkb = ", nkb  !FZ:  test
        WRITE (unit, *) "m = ", m  !FZ:  test
        WRITE (unit, *) "size (betapsi,1) = ", size (betapsi,1)  !FZ:  test
        WRITE (unit, *) "size (betapsi,2) = ", size (betapsi,2)  !FZ:  test
