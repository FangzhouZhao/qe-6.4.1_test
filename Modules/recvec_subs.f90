...
CONTAINS
  ...
  SUBROUTINE ggen ( dfftp, gamma_only, at, bg,  gcutm, ngm_g, ngm, &
    ...
    !FZ:   ig_l2g contains index of the sorting from smallest norm to largest norm
    !----------------------------------------------------------------------
    !
    ...
    iloop: DO i = istart, ni
       ...
       jloop: DO j = jstart, nj
          ...
          DO k = kstart, nk
             ...
             t(2) = ty(2) + k * bg(2,3)
             t(3) = ty(3) + k * bg(3,3)
             tt(k-kstart+1) = t(1)**2 + t(2)**2 + t(3)**2     !FZ: calculates |.|^2 of the G vector with i*Gx + j*Gy + k*Gz
          ENDDO
          !
          ...
          DO k = kstart, nk
             IF (tt(k-kstart+1) <= gcutm) THEN
                ngm = ngm + 1
                IF (ngm > ngm_max) CALL errore ('ggen 1', 'too many g-vectors', ngm)   !FZ: g2sort_g(:) has the order of expanding [i][j][k] into 1d array 
    ...
    igsrt(1) = 0
    IF( .NOT. global_sort ) THEN
       CALL hpsort_eps( ngm, g2sort_g, igsrt, eps8 )     !FZ: igsrt is the sorted index
    ELSE
